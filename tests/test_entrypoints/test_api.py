...
def test_retrieval_logging():
    ...
    with patch('dify_kg_ext.entrypoints.api.check_knowledge_exists') as mock_check, \
         patch('dify_kg_ext.entrypoints.api.retrieve_knowledge') as mock_retrieve, \
         patch('dify_kg_ext.entrypoints.api.logger') as mock_logger:
        
        mock_check.return_value = True
        mock_retrieve.return_value = {"records": [{"content": "test", "score": 0.8, "title": "test", "metadata": {}}]}
...
def test_retrieval_concurrent_requests():
    ...
    with patch('dify_kg_ext.entrypoints.api.check_knowledge_exists') as mock_check, \
        ...
        def make_request(thread_id):
            ...
            headers = {"Authorization": f"Bearer concurrent-test-key-{thread_id}-123456"}
            response = client.post("/retrieval", json=request_data, headers=headers)
